...
from models import polNet

class PolGradAgent:
    def __init__(self, env_str, batch_size, net_in, net_out, policy=polNet,  memory_len=None, env_seed=0):
        self.init_env(env_str, env_seed)
        init_actor(batch_size=batch_size)

    def init_actor(self, batch_size):
        self.actor = DQN(input_dim=(batch_size, self.env.observation_space.shape[0]),
                         output_dim=self.env.action_space[0])
    ...
    def update_actor_weights(self):
        self.actor.set_weights(self.get_model_weights())
